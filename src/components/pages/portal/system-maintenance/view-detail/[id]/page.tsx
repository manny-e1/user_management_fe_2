...
import { SysMaintenance, approveMntLogs, getMntLog, rejectMntLogs } from "@/service/system-maintenance";
			...
			>
				<div className="w-full flex gap-2 text-[#495057]">
					<div className="w-1/3">
						<div>
							...
							<div className="font-bold inline-block">: {mntLog?.submittedAt ? moment(new Date(mntLog?.submittedAt)).format("YYYY-MM-DD HH:mm:ss") : ""}</div>
						</div>
						<div>
							<div className="w-[120px] inline-block">Submitted By</div>
							<div className="font-bold inline-block">: {mntLog?.submittedBy}</div>
						</div>
					</div>
					<div className="w-1/3">
						<div>
							<div className="w-[200px] inline-block">Approved/Rejected Date</div>
							<div className="font-bold inline-block">: {(mntLog?.updatedAt && mntLog.approvedBy !== '') ? moment(new Date(mntLog?.updatedAt)).format("YYYY-MM-DD HH:mm:ss") : ""}</div>
						</div>
						<div>
							<div className="w-[200px] inline-block">Approved/Rejected By</div>
							<div className="font-bold inline-block">: {mntLog?.approvedBy}</div>
						</div>
					</div>
					<div className="w-1/3">
						<div>
						...
						<div>
							...
							<div className="font-bold inline-block">: {mntLog?.submissionStatus}</div>
						</div>
					</div>
				</div>
				<table className="mt-3 text-[#495057]">
					<tbody>
						...
						<tr>
							<td className="pe-1">
								<input type="date" className="form-control datetime-picker-size"
								value={mntLog?.startDate ? moment(new Date(mntLog?.startDate)).format("YYYY-MM-DD") : ""}
								readOnly required/>
							</td>
							<td className="px-1">
								<input type="time" className="form-control datetime-picker-size"
								value={mntLog?.startDate ? moment(new Date(mntLog?.startDate)).format("HH:mm") : ""}
								readOnly required/>
							</td>
							<td className="px-1">
								<input type="date" className="form-control datetime-picker-size"
								value={mntLog?.endDate ? moment(new Date(mntLog?.endDate)).format("YYYY-MM-DD") : ""}
								readOnly required/>
							</td>
							<td className="px-1">
								<input type="time" className="form-control datetime-picker-size"
								value={mntLog?.endDate ? moment(new Date(mntLog?.endDate)).format("HH:mm") : ""}
								readOnly required/>
							</td>
				...
				<div className="flex justify-end gap-1">
					<button type="submit" className={`bg-[#3b7ddd] hover:bg-[#326abc] active:bg-[#2f64b1] text-white px-3 py-1 rounded-[4px] flex`} onClick={handleViewLogClick}>
						View Log
					</button>
					...
					{
						(user?.role === "manager 2" && mntLog?.approvalStatus != 'Approved' && mntLog?.approvalStatus != 'Rejected') && (
							<>
								<button
									type="submit"
									id="btnApproved"
