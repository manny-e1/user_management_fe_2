...
import { SysMaintenance, SysMntInput, getMntLog, updateMntLog } from "@/service/system-maintenance";
...
export default function EditMaintenancePage() {
	...
	const handleSaveClick = () => {
		...
		tmpInput.minTime = '';
		if(tmpInput.fromDate != '')
			tmpInput.minDate = tmpInput.fromDate;
		if(tmpInput.fromDate == tmpInput.toDate) {
			if(tmpInput.fromTime != '')
				tmpInput.minTime = tmpInput.fromTime;
		}
					...
					<div className="flex justify-end gap-1">
						{
							(user?.role === "normal user 2") && (
								<>
									...
									<button type="submit" className={`bg-[#3b7ddd] hover:bg-[#326abc] active:bg-[#2f64b1] text-white px-3 py-1 rounded-[4px] flex`} onClick={handleSaveClick}>
										Save
									</button>
								</>
							)
