...
export default function DashboardPage() {
	...
	const pendQry = useQuery({
		...
		queryFn: async () => {
			const type = user?.role === 'admin' ? 'users' : (user?.role === 'manager 1' ? 'transaction' : 'maintenance');
			const result : {error: Error} | {count: number} = await getPendingCounts(type);
			if('error' in result)
				return {count: 0};
			return result;
		}
	...
	return (
		<div className="p-4 flex flex-col gap-2">
			...
			<p className="text-xl mb-2">Welcome to OBW Management and Maintenance Portal</p>
					...
					<div className="flex w-full md:w-1/2 order-1 min-[1440px]:w-1/4 px-3 mb-4">
						...
						>
							...
							<div className="flex relative font-bold text-xl justify-center text-[#6c757d] border border-[#6c757d] rounded-md w-full">
								Transaction Limits
								<div className={`${count === 0 ? 'hidden' : 'block'} absolute text-sm right-0 px-2 py-1 bg-red-600 text-white rounded-md translate-x-1/2 -translate-y-1/2`}>
									{count}
								</div>
							</div>
						</div>
