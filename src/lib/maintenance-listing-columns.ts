...
export const maintenanceListingColumns = (
	...
	columns = [
		...
		},
		{
			...
			accessorKey: 'period',
			accessorFn: (row) => {
				return moment(row.startDate).format("YYYY-MM-DD HH:mm")+"@"+moment(row.endDate).format("YYYY-MM-DD HH:mm");
			},
			cell: (props) => moment(props.row.original.startDate).format("DD/MM/YYYY HH:mm") + " - " + moment(props.row.original.endDate).format("DD/MM/YYYY HH:mm")
		},
		{
			enablePinning: true,
			...
			accessorFn: (row) => {
				let res = "";
				if(((row.iRakyatStatus != '' &&	row.approvalStatus != 'Pending' && row.approvalStatus != 'Rejected') ||
				(row.submissionStatus == 'Marked')))
					res += row.iRakyatStatus;
				if(((row.iBizRakyatStatus != '' &&	row.approvalStatus != 'Pending' && row.approvalStatus != 'Rejected') ||
				(row.submissionStatus == 'Marked')))
		...
		},
		{
			...
			cell: (props) => moment(props.row.original.submittedAt).format("DD/MM/YYYY HH:mm:ss")
 		},
		{
			header: 'Action',
			enableSorting: false,
