...
export async function getMntLogs() {
	const res = await fetch(`${API_URL}/maintenance`, { cache: 'no-cache' });
	const data: { mntLogs: SysMaintenance[] } | { error: string } = await res.json();
...
export async function createMntLog(body: {
	startDate: string,
	endDate: string,
	iRakyatYN: boolean,
	iBizRakyatYN: boolean,
...
export async function updateMntLog({
	...
	const data: {
		...
		startDate: new Date(mntInput.fromDate + " " + mntInput.fromTime).toISOString(),
...
export async function approveMntLogs({ids, email} : {ids: string[], email: string}) {
...
export async function rejectMntLogs({ids, email, msg} : {ids: string[], email: string, msg:string}) {
...
export async function completeMntLogs({id, channel} : {id: string, channel: string}) {
